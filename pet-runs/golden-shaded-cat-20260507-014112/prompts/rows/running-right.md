Create a single horizontal sprite strip for the Codex app digital pet `golden-shaded-cat` in the state `running-right`.
...
Output exactly 8 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: Golden shaded kitten based on the provided photo: compact chibi cat, oversized round dark eyes, small russet nose, cream-white muzzle and chest, warm golden shaded head and back, subtle tabby shading on forehead, fluffy triangular ears, no collar, no props..
