...
这是一个垂直切片，不是完整战役。权威经济后端是本地开发服务；GitHub Pages 默认使用本地市场模拟，任何线上后端都必须通过 HTTPS 提供。如果后端离线或被禁用，浏览器会回退到本地市场模拟，保证交易仍可游玩，依赖后端的 NPC/经济事件会平滑降级。商品、装备和派系精灵图在 UI 中通过 CSS 图集定位裁切。程序化 SFX 和 fallback 音乐刻意保持轻量，而人工整理的 BGM 覆盖范围仍限于当前 CC0 曲目集。
